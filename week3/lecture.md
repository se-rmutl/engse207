...
## Workshop/Lab (3 ชั่วโมง)

### 🎯 Workshop 1: Refactor from Monolith to Layered (60 นาที)

**วัตถุประสงค์:**
- ฝึกการออกแบบ Layered Architecture
- เข้าใจ Separation of Concerns

**โจทย์:**
ให้แต่ละกลุ่มออกแบบระบบ **Task Board System** (ระบบจัดการงานแบบ Trello) 

**ระบบ Monolithic เดิม:**
```java
public class TaskBoardApp {
    public static void main(String[] args) {
        ...
        try {
            ...
            stmt.execute("INSERT INTO tasks ...");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
1. **วาด Layered Architecture (3-tier)**
   - Presentation Layer (อะไรบ้าง?)
   - Business Logic Layer (อะไรบ้าง?)
   - Data Access Layer (อะไรบ้าง?)

2. **ระบุ Responsibilities ของแต่ละ Layer**
   - Layer ไหนทำหน้าที่อะไร?
   - มี classes/interfaces อะไรบ้าง?

3. **เขียนเป็น Pseudo Code หรือ แผนภาพ**

4. **วิเคราะห์:**
   - ข้อดีของ Layered เทียบกับ Monolith?
   - Trade-offs อะไรบ้าง?
...
### 🎯 Workshop 2: Design N-Tier for E-Commerce (60 นาที)

**วัตถุประสงค์:**
- ออกแบบ N-Tier Architecture สำหรับระบบจริง
- คำนึงถึง Quality Attributes

**โจทย์:**
ออกแบบสถาปัตยกรรมสำหรับ **ระบบ E-Commerce ขนาดกลาง** (คล้าย Shopee/Lazada)

**ความต้องการ:**
- รองรับผู้ใช้ 100,000 คนพร้อมกัน
- มี Product Catalog 1 ล้านรายการ
- ต้องการ High Availability (99.9% uptime)
- Fast page load (< 2 วินาที)

**งานที่ต้องทำ:**

1. **ออกแบบ N-Tier Architecture**
   - กำหนด Tiers ที่จำเป็น (อย่างน้อย 4 tiers)
   - ระบุ components ในแต่ละ tier

2. **วาดแผนภาพสถาปัตยกรรม**
   - ใช้ Draw.io หรือกระดาษ
   - แสดง data flow

3. **อธิบาย Quality Attributes**
   - Architecture นี้รองรับ Scalability อย่างไร?
   - จะจัดการ High Availability อย่างไร?
   - Performance optimization อะไรบ้าง?
...
### 🎯 Workshop 3: Pipe-and-Filter for Data Processing (60 นาที)

**วัตถุประสงค์:**
- ออกแบบ Pipe-and-Filter สำหรับ data pipeline
- เข้าใจการประมวลผลแบบ stream

**โจทย์:**
ออกแบบ **Log Processing Pipeline** สำหรับระบบที่มี logs จากหลาย services

**ความต้องการ:**
- รับ logs จาก 10 services
- กรอง logs ที่เป็น ERROR
- นับจำนวน errors แต่ละ service
- ส่งการแจ้งเตือนถ้า errors > 100 ต่อนาที

**งานที่ต้องทำ:**

1. **ออกแบบ Filters**
   - Filter ไหนทำอะไรบ้าง?
   - Input/Output ของแต่ละ filter?

2. **วาด Pipeline Diagram**
   - เรียง filters อย่างไร?
   - มี pipes เชื่อมต่ออย่างไร?

3. **เขียน Pseudo Code**
   - อธิบายการทำงานของแต่ละ filter

4. **วิเคราะห์:**
   - ข้อดีของ Pipe-and-Filter สำหรับงานนี้?
   - มีข้อจำกัดอะไรบ้าง?
...
- Keep only ERROR logs
    ↓
[Filter 3: Group by Service]
- Count errors per service
    ↓
[Filter 4: Aggregate by Time Window]
- Sum errors per 1-minute window
    ↓
[Filter 5: Alert Decision]
